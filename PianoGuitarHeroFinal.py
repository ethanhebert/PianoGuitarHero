    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (createNotes == 1):
                ...
                #not on the keyboard, but more notes to use with guitar
                B4 = Note(493.88, 1, form)
                C5 = Note(523.25, 1, form) 
                C5s = Note(554.37, 1, form)
                D5 = Note(587.33, 1, form)
                D5s = Note(622.25, 1, form)
                E5 = Note(659.26, 1, form)
                F5 = Note(698.46, 1, form)
                F5s = Note(739.99, 1, form)
                G5 = Note(783.99, 1, form)
                G5s = Note(830.61, 1, form)
                A5 = Note(880.00, 1, form)
                A5s = Note(932.33, 1, form)
        ...
        if controller1 == "1-player WUSBMote v2.2":
            if event.type == pygame.JOYBUTTONDOWN and notesCreated == 1:
                ...
                #COME AS YOU ARE - NIRVANA (G4, D4, D4s E4, A4)
                """
                if event.button == 0: #green
                    G4.play(-1)
                if event.button == 1: #red
                    D4.play(-1)
                if event.button == 2: #yellow
                    D4s.play(-1)
                if event.button == 3: #blue
                    E4.play(-1)         
                if event.button == 4: #orange
                    A4.play(-1)
                """

                #WALK OF LIFE - DIRE STRAITS (E4, F4s, G4s, B4)
                if event.button == 0: #green
                    E4.play(-1)
                if event.button == 1: #red
                    F4s.play(-1)
